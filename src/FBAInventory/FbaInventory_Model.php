...
/**
 * FbaInventory_Model - base class for all model classes
 */
abstract class FbaInventory_Model
{
    ...
    */
    ...
    protected function _toXMLFragment() 
    {
        ...
        foreach ($this->_fields as $fieldName => $field) {
            ...
            if (!is_null($fieldValue) && $field['FieldType'] != "FbaInventory_Model_ResponseHeaderMetadata") {
                $fieldType = $field['FieldType'];
                if (is_array($fieldType)) {
                    if ($fieldType[0] == "object") {
