 ...
 *
 */
class FbaInbound_Client implements FbaInbound_Interface
{
   ...
   private function collapseWhitespace($s) {
    ...
    private function _invoke(array $parameters)
    {
        try {
            if (empty($this->_config['ServiceURL'])) {
                ...
                throw new FbaInbound_Exception(
        ...
        } catch (FbaInbound_Exception $se) {
            throw $se;
        } catch (\Exception $t) {
            //require_once (dirname(__FILE__) . '/Exception.php');
            throw new FbaInbound_Exception(array('Exception' => $t, 'Message' => $t->getMessage()));
        }
    }
    ...
    private function _reportAnyErrors($responseBody, $status, $responseHeaderMetadata, Exception $e =  null)
    {
        ...
        //require_once (dirname(__FILE__) . '/Exception.php');
        return new FbaInbound_Exception($exProps);
    }
    ...
    private function _httpPost(array $parameters)
    {
        ...
        if($response === false) {
            ...
            curl_close($ch);
            throw new FbaInbound_Exception($exProps);
        }
