...
abstract class Cart_Model
{
    ...
    /**
     * @param null $data - DOMElement or Associative Array to construct from.
    ...
    public function __construct($data = null)
    {
        if (!is_null($data)) {
            if ($this->_isAssociativeArray($data)) {
                ...
                throw new Cart_Exception ("Unable to construct from provided data. Please be sure to pass associative array or DOMElement");
            }
        }
    }
    ...
    */
    ...
    protected function _toXMLFragment() 
    {
        ...
        foreach ($this->_fields as $fieldName => $field) {
            ...
            if (!is_null($fieldValue) && $field['FieldType'] != "Cart_Model_ResponseHeaderMetadata") {
                $fieldType = $field['FieldType'];
                if (is_array($fieldType)) {
                    if ($fieldType[0] == "object") {
