 ...
 *
 */
class Feeds_Client implements Feeds_Interface
{
      ...
      do {
        try {
          ...
          //require_once ('MarketplaceWebService/Exception.php');
            throw new Feeds_Exception(array('Exception' => $e, 'Message' => $e->getMessage()));
        }

      } while ($shouldRetry);

    } catch (Feeds_Exception $se) {
      throw $se;
    } catch (\Exception $t) {
      throw new Feeds_Exception(array('Exception' => $t, 'Message' => $t->getMessage()));
    }
  ...
  private function reportAnyErrors($responseBody, $status, $responseHeaderMetadata)
  {
    ...
    //require_once ('MarketplaceWebService/Exception.php');
    return new Feeds_Exception($exProps);
  }
  ...
  private function performRequest($action, array $converted, $dataHandle = null, $contentMd5 = null) {
    ...
    if (Feeds_RequestType::getRequestType($action) === Feeds_RequestType::POST_DOWNLOAD) {
    	if ($code != 200) {
...
//  	$responseHandle;
  ...
  private function configureCurlOptions($action, array $converted, $streamHandle = null, $contentMd5 = null) {
    ...
    $requestType = Feeds_RequestType::getRequestType($action);

    if ($requestType == Feeds_RequestType::POST_UPLOAD) {
    ...
    } else if (!($requestType === Feeds_RequestType::UNKNOWN)) {
      ...
      if ($requestType == Feeds_RequestType::POST_DOWNLOAD) {
